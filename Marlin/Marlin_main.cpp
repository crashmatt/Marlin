...
bool axis_known_position[4] = { false };
...
float home_offset[4] = { 0 };
float min_pos[4] = { X_MIN_POS, Y_MIN_POS, X_MIN_POS, Y_MIN_POS };
float max_pos[4] = { X_MAX_POS, Y_MAX_POS, X_MAX_POS, Y_MAX_POS };
...
const char axis_codes[NUM_AXIS] = {'X', 'Y', 'U', 'V'};
...
#define XYZ_CONSTS_FROM_CONFIG(type, array, CONFIG) \
  static const PROGMEM type array##_P[4] =        \
      { X_##CONFIG, Y_##CONFIG, U_##CONFIG, V_##CONFIG };     \
...
inline void line_to_current_position() {
  plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[U_AXIS], current_position[V_AXIS], feedrate / 60, active_extruder);
}
inline void line_to_destination(float mm_m) {
  plan_buffer_line(destination[X_AXIS], destination[Y_AXIS], destination[U_AXIS], destination[V_AXIS], mm_m / 60, active_extruder);
}
...
static void home_multi_axis(uint8_t home_axis) {
	uint8_t axis_done = 0;
	bool axis_hit;
	bool axis_todo;
	int  axis;
	char endstop_hit_bits;

	#if ENABLED(DEBUG_LEVELING_FEATURE)
		if (marlin_debug_flags & DEBUG_LEVELING) {
			SERIAL_ECHOPAIR(">>> home_mult_axis(", (unsigned long)home_axis);
			SERIAL_CHAR(')');
			SERIAL_EOL;
		}
	#endif
	...
	// Loops around enough times to approximate home all axis
	do {
		// Check if each axis is done on each loop
		axis_todo = false;
		feedrate = homing_feedrate[0];

		// Set all active axis position to zero as setup for the move
		for(int axis=0; axis<NUM_AXIS; axis++){
			if(home_axis & BIT(axis)){
		...
		for(axis=0; axis<NUM_AXIS; axis++){
			// If axis is done then don't move it
			if(!(axis_done & BIT(axis))){
				if(home_axis & BIT(axis)){
					axis_todo = true;
		...
		if(axis_todo == true){
			...
			// Endstop hit - Set position and destination of all active axis to zero
			for(axis=0; axis<NUM_AXIS; axis++){
				if(home_axis & BIT(axis)){
					current_position[axis] = 0;
					destination[axis] = 0;
			...
			char endstop_hit_bits = get_endstop_hit_bits();
			// Set the axis position as setup for the move
			if(endstop_hit_bits & (BIT(X_MIN)|BIT(X_MAX)) ){
				destination[X_AXIS] = -home_bump_mm(X_AXIS) * home_dir(X_AXIS);
				axis_done |= BIT(X_AXIS);
			}
			if(endstop_hit_bits & (BIT(Y_MIN)|BIT(Y_MAX))){
				destination[Y_AXIS] = -home_bump_mm(Y_AXIS) * home_dir(Y_AXIS);
				axis_done |= BIT(Y_AXIS);
			}
			if(endstop_hit_bits & (BIT(U_MIN)|BIT(U_MAX))){
				destination[U_AXIS] = -home_bump_mm(U_AXIS) * home_dir(U_AXIS);
				axis_done |= BIT(U_AXIS);
			}
			if(endstop_hit_bits & (BIT(V_MIN)|BIT(V_MAX))){
				destination[V_AXIS] = -home_bump_mm(V_AXIS) * home_dir(V_AXIS);
				axis_done |= BIT(V_AXIS);
			}
	    ...
	    st_synchronize();
			...
			endstops_hit_on_purpose();	// Clear the endstop flags
	...
	} while (axis_todo == true);

//	//Slow final homing of each axis individually
//	for(axis=0; axis<NUM_AXIS; axis++){
//		// Only do required axis
//		if(home_axis & BIT(axis)){
//			destination[axis] = 1.5 * max_length(axis) * home_dir(axis);
//
//		}
//	}
		...
		// Set all active axis position to zero to finish
		for(int axis=0; axis<NUM_AXIS; axis++){
			if(home_axis & BIT(axis)){
					current_position[axis] = 0;
					destination[axis] = 0;
 ...
 *  S 	Home XYUV vertical and horizontal axis separately
 *  M 	Home XYUV vertical and horizontal axis together
 *
 */
inline void gcode_G28() {
  ...
  if(code_seen('M')){
    ...
    set_destination_to_current();
		home_multi_axis(BIT(X_AXIS) | BIT(Y_AXIS) | BIT(U_AXIS) | BIT(V_AXIS));
  }
  ...
  if(code_seen('S')){
    ...
    set_destination_to_current();
		...
		home_multi_axis(BIT(Y_AXIS) | BIT(V_AXIS));
		#endif

		home_multi_axis(BIT(X_AXIS) | BIT(U_AXIS));
  ...
  feedrate = 0.0;
	...
	bool  homeX = code_seen(axis_codes[X_AXIS]),
				...
				homeU = code_seen(axis_codes[U_AXIS]),
				homeV = code_seen(axis_codes[V_AXIS]);

	home_all_axis = (!homeX && !homeY && !homeU && !homeV) || (homeX && homeY && homeU && homeV);
...
inline void gcode_M18_M84() {
  if (code_seen('S')) {
    ...
    bool all_axis = !((code_seen(axis_codes[X_AXIS])) || (code_seen(axis_codes[Y_AXIS])) || (code_seen(axis_codes[U_AXIS])) || (code_seen(axis_codes[V_AXIS])));
    if (all_axis) {
      finishAndDisableSteppers();
    }
    else {
...
inline void gcode_M114() {
  ...
  SERIAL_PROTOCOLPGM(" U:");
  SERIAL_PROTOCOL(current_position[U_AXIS]);
  SERIAL_PROTOCOLPGM(" V:");
  SERIAL_PROTOCOL(current_position[V_AXIS]);
  ...
  SERIAL_PROTOCOLPGM(" U:");
  SERIAL_PROTOCOL(st_get_position_mm(U_AXIS));
  SERIAL_PROTOCOLPGM(" V:");
  SERIAL_PROTOCOL(st_get_position_mm(V_AXIS));
...
inline void gcode_M119() {
  SERIAL_PROTOCOLLN(MSG_M119_REPORT);
	...
	#if HAS_U_MIN
		SERIAL_PROTOCOLPGM(MSG_U_MIN);
		SERIAL_PROTOCOLLN(((READ(U_MIN_PIN)^U_MIN_ENDSTOP_INVERTING) ? MSG_ENDSTOP_HIT : MSG_ENDSTOP_OPEN));
	#endif
	#if HAS_U_MAX
		SERIAL_PROTOCOLPGM(MSG_U_MAX);
		SERIAL_PROTOCOLLN(((READ(U_MAX_PIN)^U_MAX_ENDSTOP_INVERTING) ? MSG_ENDSTOP_HIT : MSG_ENDSTOP_OPEN));
	#endif
	#if HAS_V_MIN
		SERIAL_PROTOCOLPGM(MSG_V_MIN);
		SERIAL_PROTOCOLLN(((READ(V_MIN_PIN)^V_MIN_ENDSTOP_INVERTING) ? MSG_ENDSTOP_HIT : MSG_ENDSTOP_OPEN));
	#endif
	#if HAS_V_MAX
		SERIAL_PROTOCOLPGM(MSG_V_MAX);
		SERIAL_PROTOCOLLN(((READ(V_MAX_PIN)^V_MAX_ENDSTOP_INVERTING) ? MSG_ENDSTOP_HIT : MSG_ENDSTOP_OPEN));
	#endif
...
inline void gcode_M206() {
  for (int8_t i = X_AXIS; i <= V_AXIS; i++) {
    if (code_seen(axis_codes[i])) {
...
void clamp_to_software_endstops(float target[3]) {
  if (min_software_endstops) {
    ...
    NOLESS(target[U_AXIS], min_pos[U_AXIS]);
    NOLESS(target[V_AXIS], min_pos[V_AXIS]);
  }

  if (max_software_endstops) {
    NOMORE(target[X_AXIS], max_pos[X_AXIS]);
    NOMORE(target[Y_AXIS], max_pos[Y_AXIS]);
    NOMORE(target[U_AXIS], max_pos[U_AXIS]);
    NOMORE(target[V_AXIS], max_pos[V_AXIS]);
  }
}
